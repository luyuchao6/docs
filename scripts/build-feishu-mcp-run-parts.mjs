...
const chunks = [manifest.slice(0, 9), manifest.slice(9, 17), manifest.slice(17, 25)];

function makeCode(manifestLiteral) {
  return `async (page) => {
...
chunks.forEach((c, i) => {
  const code = makeCode(JSON.stringify(c));
  const out = path.join(__dirname, `.feishu-mcp-run-part${i}.json`);
